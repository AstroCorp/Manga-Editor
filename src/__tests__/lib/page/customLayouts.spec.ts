...
describe('customLayouts', () => {
	it('createCustomLayoutEntry strips images and uses page stroke', () => {
		const entry = createCustomLayoutEntry(sampleLayout());

		expect(entry.id).toMatch(
			/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i,
		);
		expect(entry.layout.shapes).toHaveLength(1);
		expect(entry.layout.shapes?.[0]?.image).toBeNull();
		expect(entry.layout.shapes?.[0]?.strokeWidth).toBe(5);
		expect(entry.layout.strokeWidth).toBe(5);
	});
	...
	it('serializer round-trips valid custom layouts', () => {
		...
		expect(read[0]?.layout.shapes?.[0]?.image).toBeNull();
	});
	...
	it('serializer returns empty array for invalid payloads', () => {
		...
		expect(
			customLayoutsSerializer.read(
				JSON.stringify([
					{ id: 'ok', layout: { width: 100, height: 200 } },
					{ id: 'bad', layout: null },
				]),
			),
		).toHaveLength(1);
